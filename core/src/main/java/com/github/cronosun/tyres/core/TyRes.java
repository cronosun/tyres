    ...
    private static TyResImplementation determineImplementation() {
        ...
        if (iterator.hasNext()) {
            ...
            if (iterator.hasNext()) {
                ...
                throw new TyResException("There are at least two implementations of TyRes on the classpath. To " +
                        "make sure the application produces reproducible results, only one implementation is " +
                        "supported. Found implementations (might have more): [" + implementation.getClass() + ", " +
                        secondImplementation.getClass() + "].");
            }
            return implementation;
        } else {
            throw new TyResException("No tyres implementation found. Make sure exactly one implementation of " +
