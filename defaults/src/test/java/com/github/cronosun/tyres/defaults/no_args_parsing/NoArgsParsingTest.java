...
public class NoArgsParsingTest {
    ...
    void msgResAlwaysUsesMsgFormatWhileStrResNeverUsesMsgFormat() {
        ...
        assertEquals("The  character must be escaped in messages if you want this character.", resources.resolver().get(NoArgsParsingBundle.INSTANCE.msgRes(), Locale.ROOT));
        assertEquals("The ' character must be escaped in messages if you want this character.", resources.resolver().get(NoArgsParsingBundle.INSTANCE.strRes(), Locale.ROOT));
    }
}
